...
from hiddenlayer import HiddenLayer, AsyncHiddenLayer, DefaultAioHttpClient
from hiddenlayer._utils import is_dict
from hiddenlayer._oauth2 import OAuth2ClientCredentials
...
logging.getLogger("hiddenlayer").setLevel(logging.DEBUG)
...
def client(request: FixtureRequest) -> Iterator[HiddenLayer]:
    strict = getattr(request, "param", True)
    if not isinstance(strict, bool):
        raise TypeError(f"Unexpected fixture parameter type {type(strict)}, expected {bool}")

    with HiddenLayer(base_url=base_url, bearer_token=bearer_token, _strict_response_validation=strict) as client:
        yield client


@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncHiddenLayer]:
